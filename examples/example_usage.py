...
from src.tools.analysis.model_handler import ModelHandler
from src.tools.analysis.analysis import AnalysisEngine
from src.tools.analysis.output import OutputManager
import torch
import logging
...
def example_text_analysis():
    ...
    results = analysis_engine.run_full_analysis(
        model_output, layer_outputs, attention_weights, analysis_config
    )
    ...
    if "model_output" in results and "top_tokens" in results["model_output"]:
        ...
        print(
            f"Top prediction: {top_tokens['tokens'][0]} ({top_tokens['probabilities'][0][0]:.3f})"
        )
...
def example_vision_analysis():
    ...
    try:
        ...
        print(
            f"Vision model example failed (this might be expected if model not available): {e}"
        )
    ...
    for i, input_text in enumerate(test_inputs):
        ...
        if model_output.get("logits") is not None:
            ...
            confidence = confidence_calc.calculate_prediction_confidence(
                model_output["logits"], method="all"
            )
...
def example_layer_comparison():
    ...
    for layer_idx in sorted(layer_outputs.keys()):
        ...
        if "hidden_states" in layer_data:
            ...
            print(
                f"Layer {layer_idx}: shape {list(hidden.shape)}, norm {hidden.norm():.2f}"
            )
...
def example_save_results():
    ...
    json_file = output_manager.save_results(
        results, "distilgpt2_example", format="json"
    )
    print(f"Results saved as JSON: {json_file}")

    yaml_file = output_manager.save_results(
        results, "distilgpt2_example", format="yaml"
    )
    print(f"Results saved as YAML: {yaml_file}")

    # Create summary report
    report_file = output_manager.create_summary_report(
        results, "distilgpt2_example", input_text
    )
    ...
    if attention_weights:
        plot_files = output_manager.save_attention_plots(
            attention_weights, analysis_engine.attention_analyzer, "distilgpt2_example"
        )
