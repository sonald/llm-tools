...
| `tokenizer.json` | 结构/词表分析、Raw 与 Chat 工作台 | 同目录 bundle 最多 32 MiB；输入最多 64 KiB；Worker latest-only |
...
不支持 ModelScope（浏览器清单 CORS 门失败）、SSH、私有仓库/token、模型推理、转换/量化/上传或完整权重下载；不会引入后端、代理、浏览器扩展或桌面桥接。完整支持矩阵和证据见 [产品化计划](docs/productization-plan.md) 与 [本地功能完整性验收记录](docs/local-functionality-acceptance.md)。
