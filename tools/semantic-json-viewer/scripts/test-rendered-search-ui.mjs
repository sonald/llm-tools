...
const makeSearch=({invoke=async()=>({matches:[],hasMore:false,nextCursor:null}),onReveal=()=>{},onError=()=>{},onIntentChange=()=>{},onProjectionUnavailable=()=>{},projectionBudget}= {})=>{
  ...
  const view=new RenderedSearch({...elements,invoke,onReveal,onError,onIntentChange,onProjectionUnavailable,projectionBudget});
...
const releasedRoot=document.createElement("div");
releasedRoot.textContent="release target";
document.body.append(releasedRoot);
const released=makeSearch();
released.view.activateDom(null,releasedRoot,"Release rendered search resources.");
await settle();
released.elements.query.value="release";
released.elements.form.requestSubmit();
await settle();
released.elements.results.querySelector("button")?.click();
check(releasedRoot.querySelector("mark[data-rendered-search]")!==null,"Rendered owner cleanup fixture did not create a highlight");
released.view.clear(false);
check(released.view.projection===null&&released.view.domRoot===null&&released.view.domSnapshot.length===0&&released.view.marks.length===0&&released.view.textRestores.length===0&&released.view.projectionAbort===null&&released.view.projectionPending===false,"Rendered owner clear retained projection, DOM or highlight references");
check(releasedRoot.innerHTML==="release target","Rendered owner clear did not restore the original DOM");
released.host.remove();
releasedRoot.remove();

const nonOwnerRoot=document.createElement("div");
nonOwnerRoot.textContent="old target";
document.body.append(nonOwnerRoot);
const nonOwner=makeSearch();
nonOwner.view.activateDom(null,nonOwnerRoot,"Release rendered resources without owning the shared search.");
await settle();
nonOwner.elements.query.value="old";
nonOwner.elements.form.requestSubmit();
await settle();
nonOwner.elements.results.querySelector("button")?.click();
const sourceResult=document.createElement("button");
sourceResult.type="button";
sourceResult.textContent="source result";
nonOwner.elements.results.replaceChildren(sourceResult);
nonOwner.elements.resultsPanel.hidden=false;
nonOwner.elements.status.textContent="source status";
nonOwner.elements.query.value="source query";
nonOwner.elements.form.dataset.searchOwner="source";
const newHighlight=document.createElement("mark");
newHighlight.dataset.renderedSearch="true";
newHighlight.textContent="new highlight";
nonOwnerRoot.replaceChildren(newHighlight);
nonOwner.view.clear(false);
check(nonOwner.elements.form.dataset.searchOwner==="source"&&nonOwner.elements.status.textContent==="source status"&&nonOwner.elements.query.value==="source query"&&nonOwner.elements.results.firstChild===sourceResult,"Non-owner rendered clear damaged the source search controls");
check(nonOwnerRoot.firstChild===newHighlight&&newHighlight.isConnected,"Non-owner rendered clear removed a newer DOM/highlight");
check(nonOwner.view.projection===null&&nonOwner.view.domRoot===null&&nonOwner.view.domSnapshot.length===0&&nonOwner.view.marks.length===0&&nonOwner.view.textRestores.length===0,"Non-owner rendered clear retained old DOM or highlight references");
nonOwner.host.remove();
nonOwnerRoot.remove();

const lateProjectionRoot=document.createElement("div");
lateProjectionRoot.textContent="x ".repeat(200000);
document.body.append(lateProjectionRoot);
const lateProjection=makeSearch();
lateProjection.view.activateDom(null,lateProjectionRoot,"Cancel a pending rendered projection.");
await Promise.resolve();
check(lateProjection.view.projectionPending===true,"Pending rendered projection was not observable before clear");
lateProjection.view.clear(false);
await settle();
check(lateProjection.view.projection===null&&lateProjection.view.domRoot===null&&lateProjection.view.domSnapshot.length===0&&lateProjection.view.projectionAbort===null&&lateProjection.view.projectionPending===false,"Late rendered projection response resurrected cleared references");
lateProjection.host.remove();
lateProjectionRoot.remove();

const lateRevealCalls=[];
const lateReveal=makeSearch({invoke:async()=>new Promise((resolve)=>lateRevealCalls.push(resolve))});
lateReveal.view.activate("backend",{nodeId:19,scopeId:null,sessionRevision:1,scopeStart:0,scopeEnd:100},null,"Cancel a pending rendered reveal.");
lateReveal.elements.query.value="late";
lateReveal.elements.form.requestSubmit();
await Promise.resolve();
check(lateRevealCalls.length===1,"Pending rendered reveal request was not started");
lateReveal.view.clear(false);
lateRevealCalls[0]({matches:[],hasMore:false,nextCursor:null});
await settle();
check(lateReveal.view.projection===null&&lateReveal.view.domRoot===null&&lateReveal.view.domSnapshot.length===0&&lateReveal.elements.results.children.length===0&&lateReveal.elements.resultsPanel.hidden,"Late rendered reveal response repopulated cleared search state");
lateReveal.host.remove();

const {SearchView}=await import("/src/search-view.ts");
const sharedProjectionBudget=new ProjectionBudget(800);
const sourceBudgetHost=document.createElement("div");
sourceBudgetHost.innerHTML='<form><input id="query"><input id="decoded" type="radio" checked><input id="raw" type="radio"><button id="submit" type="submit">Search</button><p id="description"></p></form><div id="panel"></div><div id="results-panel"><div id="status"></div><div id="results"></div><button id="previous">Previous</button><button id="next">Next</button></div>';
document.body.append(sourceBudgetHost);
const sourceBudgetElements={form:sourceBudgetHost.querySelector("form"),query:sourceBudgetHost.querySelector("#query"),decoded:sourceBudgetHost.querySelector("#decoded"),rawSource:sourceBudgetHost.querySelector("#raw"),submit:sourceBudgetHost.querySelector("#submit"),description:sourceBudgetHost.querySelector("#description"),panel:sourceBudgetHost.querySelector("#panel"),resultsPanel:sourceBudgetHost.querySelector("#results-panel"),status:sourceBudgetHost.querySelector("#status"),results:sourceBudgetHost.querySelector("#results"),previous:sourceBudgetHost.querySelector("#previous"),next:sourceBudgetHost.querySelector("#next")};
const sourceBudgetScope={label:"Document root",description:"Source Search",enabled:true,decodedEnabled:true,scopeStart:0,scopeEnd:1000,sessionRevision:3,scopeId:null,targetNodeId:null};
const sourceBudgetView=new SearchView({...sourceBudgetElements,projectionBudget:sharedProjectionBudget,invoke:async()=>({matches:[{nodeId:4,field:"value",pathSegments:["$","message"],pathTruncated:false,sourceSpanStart:20,sourceSpanEnd:42,matchStart:0,matchEnd:6}],hasMore:false,nextCursor:null}),onReveal:()=>{},onError:(error)=>{throw error;}});
sourceBudgetView.setScope(sourceBudgetScope);
sourceBudgetElements.query.value="needle";
sourceBudgetElements.form.requestSubmit();
await settle();
check(sharedProjectionBudget.usedBytes>0,"Source Search did not occupy the shared projection budget");
const rejectedRenderedRoot=document.createElement("div");
rejectedRenderedRoot.textContent="rendered";
document.body.append(rejectedRenderedRoot);
const rejectedRendered=makeSearch({projectionBudget:sharedProjectionBudget});
rejectedRendered.view.activateDom(null,rejectedRenderedRoot,"Shared projection budget rejection.");
await settle();
check(rejectedRendered.view.projection===null&&sharedProjectionBudget.usedBytes>0&&sourceBudgetElements.form.dataset.searchOwner==="source","Rendered projection admission displaced the current Source Search page");
rejectedRendered.host.remove();
rejectedRenderedRoot.remove();
sourceBudgetHost.remove();

const evictionBudget=new ProjectionBudget(2000);
let evictionFallbacks=0;
const evictionRoot=document.createElement("div");
evictionRoot.textContent="evictable rendered projection";
document.body.append(evictionRoot);
const evictable=makeSearch({projectionBudget:evictionBudget,onProjectionUnavailable:()=>{evictionFallbacks+=1;}});
evictable.view.activateDom(null,evictionRoot,"Evictable rendered projection.");
await settle();
check(evictable.view.projection!==null&&evictionBudget.usedBytes>0,"Rendered projection was not admitted to the shared budget");
const externalProjection={};
check(evictionBudget.admit(externalProjection,1900,()=>{}),"External projection pressure could not evict the rendered projection");
check(evictable.view.projection===null&&evictionFallbacks===1&&evictionBudget.usedBytes===1900,"Evicted rendered projection did not release its ledger entry and fallback once");
evictable.host.remove();
evictionRoot.remove();

const lruBudgetA=new ProjectionBudget(400);
const lruA=makeSearch({projectionBudget:lruBudgetA});
const lruB=makeSearch({projectionBudget:lruBudgetA});
const lruRootA=document.createElement("div");
const lruRootB=document.createElement("div");
lruRootA.textContent="A";
lruRootB.textContent="B";
document.body.append(lruRootA,lruRootB);
lruA.view.activateDom(null,lruRootA,"LRU projection A.");
lruB.view.activateDom(null,lruRootB,"LRU projection B.");
await settle();
lruA.view.highlightRange(0,1);
const lruPressureA={};
check(lruBudgetA.admit(lruPressureA,200,()=>{}),"LRU pressure A was not admitted");
check(lruA.view.projection!==null&&lruB.view.projection===null,"Touching projection A did not preserve A over older projection B");
lruA.view.clear(false);
lruB.view.clear(false);
lruBudgetA.release(lruPressureA);
lruA.host.remove();
lruB.host.remove();
lruRootA.remove();
lruRootB.remove();

const lruBudgetB=new ProjectionBudget(400);
const lruC=makeSearch({projectionBudget:lruBudgetB});
const lruD=makeSearch({projectionBudget:lruBudgetB});
const lruRootC=document.createElement("div");
const lruRootD=document.createElement("div");
lruRootC.textContent="C";
lruRootD.textContent="D";
document.body.append(lruRootC,lruRootD);
lruC.view.activateDom(null,lruRootC,"LRU projection C.");
lruD.view.activateDom(null,lruRootD,"LRU projection D.");
await settle();
lruD.view.highlightRange(0,1);
const lruPressureB={};
check(lruBudgetB.admit(lruPressureB,200,()=>{}),"LRU pressure B was not admitted");
check(lruC.view.projection===null&&lruD.view.projection!==null,"Touching projection D did not preserve D over older projection C");
lruC.view.clear(false);
lruD.view.clear(false);
lruBudgetB.release(lruPressureB);
lruC.host.remove();
lruD.host.remove();
lruRootC.remove();
lruRootD.remove();

const refreshBudget=new ProjectionBudget(2000);
const refreshView=makeSearch({projectionBudget:refreshBudget});
const refreshRootA=document.createElement("div");
const refreshRootB=document.createElement("div");
refreshRootA.textContent="A";
refreshRootB.textContent="B".repeat(20);
document.body.append(refreshRootA,refreshRootB);
refreshView.view.activateDom(null,refreshRootA,"Refresh projection A.");
await settle();
const refreshProjectionB=await projectRenderedText(refreshRootB);
refreshView.view.refresh(refreshProjectionB);
const refreshBytes=refreshProjectionB.text.length*2+refreshProjectionB.segments.length*128;
check(refreshBudget.usedBytes===refreshBytes,"Replacing a rendered projection retained the old ledger entry");
refreshView.view.clear(false);
check(refreshBudget.usedBytes===0,"Clearing after replacing a rendered projection did not release the ledger");
refreshView.host.remove();
refreshRootA.remove();
refreshRootB.remove();

const staleRefreshBudget=new ProjectionBudget(600);
const staleRefresh=makeSearch({projectionBudget:staleRefreshBudget});
const staleRootA=document.createElement("div");
const staleRootB=document.createElement("div");
const staleRootC=document.createElement("div");
staleRootA.textContent="A";
staleRootB.textContent="B".repeat(20);
staleRootC.textContent="C".repeat(20);
document.body.append(staleRootA,staleRootB,staleRootC);
staleRefresh.view.activateDom(null,staleRootA,"Stale refresh source.");
await settle();
const staleProjection=staleRefresh.view.projection;
check(staleProjection!==null,"Stale refresh fixture did not admit its original projection");
staleRefreshBudget.release(staleProjection);
const staleExternal={};
check(staleRefreshBudget.admit(staleExternal,500,()=>staleRefresh.view.activateDom(null,staleRootC,"Replacement same-target root.")),"Stale refresh pressure was not admitted");
staleRefresh.view.refresh(await projectRenderedText(staleRootB));
await settle();
check(staleRefresh.view.activeMode==="dom"&&staleRefresh.view.domRoot===staleRootC&&staleRefresh.view.projection?.text===staleRootC.textContent&&!staleRefresh.view.busy,"Stale refresh admission cleared the replacement same-target root");
staleRefresh.host.remove();
staleRootA.remove();
staleRootB.remove();
staleRootC.remove();
staleRefresh.view.clear(false);
check(staleRefreshBudget.usedBytes===0,"Stale refresh replacement did not release the final projection ledger");
