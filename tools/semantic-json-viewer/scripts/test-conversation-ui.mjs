...
const wrapper=(candidate,scopeRootId=1)=>({scopeRootId,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:candidate.id,candidateSpanStart:candidate.spanStart,candidateSpanEnd:candidate.spanEnd,ambiguousDuplicateField:false});
...
const ambiguousRoot=node(8000,"object","$",0,1000,1);const ambiguousCandidate=node(8010,"array","messages",100,900,1);const ambiguousRootRef=ref(8000,0,1000);const ambiguousCandidateDto={nodeId:8010,spanStart:100,spanEnd:900,messageCount:1,kind:"none",scopeRootId:8000,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:30,ambiguousDuplicateField:true};const ambiguousWrapperPage={blocks:[{kind:"source",messageNodeId:null,messageSpanStart:null,messageSpanEnd:null,sourceNodeId:8000,sourceSpanStart:0,sourceSpanEnd:1000,fieldNodeId:null,fieldSpanStart:null,fieldSpanEnd:null,category:"unknown",role:"unknown",roleSourceNodeId:null,roleSourceSpanStart:null,roleSourceSpanEnd:null,openaiRefs:null,anthropicRefs:null,ambiguousDuplicateField:true}],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:8000,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:8010,candidateSpanStart:100,candidateSpanEnd:900,ambiguousDuplicateField:true}};
const ambiguousWrapperCalls=[];const ambiguousWrapperInvoke=async(command,args)=>{ambiguousWrapperCalls.push({command,args});if(command==="get_children")return {nodes:[ambiguousCandidate],hasMore:false,nextCursor:null};if(command==="get_conversation_candidate")return ambiguousCandidateDto;if(command==="get_conversation_blocks")return ambiguousWrapperPage;if(command==="get_node_summary")return node(args.nodeId,"object","ambiguous wrapper",0,1000,0);throw new Error("unexpected ambiguous wrapper command "+command);};
const ambiguousWrapperHost=document.createElement("div");document.body.append(ambiguousWrapperHost);const ambiguousWrapperView=new ConversationView({panel:ambiguousWrapperHost,invoke:ambiguousWrapperInvoke,onError:(error)=>{throw error;},onRaw:(target)=>raw.push(target),onTree:(target)=>tree.push(target),onContent:(target)=>content.push(target)});ambiguousWrapperView.setContext({mode:"document",sessionRevision:30,sourceSize:1000,scopeRoot:ambiguousRoot,scopeLabel:"ambiguous wrapper"});await settle();await settle();check(ambiguousWrapperCalls.some((call)=>call.command==="get_conversation_blocks"&&call.args.style==="generic")&&ambiguousWrapperHost.querySelectorAll(".conversation-block").length===1,"ambiguous none candidate was hidden instead of entering Generic source fallback");check(ambiguousWrapperHost.textContent.includes("Ambiguous duplicate field")||ambiguousWrapperHost.textContent.includes("重复字段"),"ambiguous wrapper did not show its localized duplicate-field warning");check(ambiguousWrapperHost.querySelector('[data-conversation-action="wrapper-raw"]')!==null&&ambiguousWrapperHost.querySelector('[data-conversation-action="wrapper-tree"]')!==null&&ambiguousWrapperHost.querySelector(".conversation-inline-content")==null,"ambiguous wrapper did not expose only complete source Raw/Tree actions");ambiguousWrapperHost.querySelector('[data-conversation-action="wrapper-raw"]').click();check(raw.at(-1)?.ref.nodeId===8000&&raw.at(-1)?.ref.spanStart===0,"ambiguous wrapper Raw action did not target the complete scope root");ambiguousWrapperHost.querySelector('[data-conversation-action="wrapper-tree"]').click();check(tree.at(-1)?.ref.nodeId===8000,"ambiguous wrapper Tree action did not target the complete scope root");ambiguousWrapperHost.remove();

const ambiguousMessageRoot=node(8100,"object","$",0,1000,1);const ambiguousMessageCandidate=node(8200,"array","messages",100,900,4);const wholeMessage=ref(8210,120,600);const childSource=ref(8220,200,300);const childField=ref(8230,150,350);const roleSource=ref(8240,130,145);const ambiguousMessageBlock={kind:"source",messageNodeId:8210,messageSpanStart:120,messageSpanEnd:600,sourceNodeId:8210,sourceSpanStart:120,sourceSpanEnd:600,fieldNodeId:null,fieldSpanStart:null,fieldSpanEnd:null,category:"unknown",role:"unknown",roleSourceNodeId:null,roleSourceSpanStart:null,roleSourceSpanEnd:null,openaiRefs:null,anthropicRefs:null,ambiguousDuplicateField:true};const ambiguousChildBlock={kind:"source",messageNodeId:8210,messageSpanStart:120,messageSpanEnd:600,sourceNodeId:8220,sourceSpanStart:200,sourceSpanEnd:300,fieldNodeId:8230,fieldSpanStart:150,fieldSpanEnd:350,category:"unknown",role:"assistant",roleSourceNodeId:8240,roleSourceSpanStart:130,roleSourceSpanEnd:145,openaiRefs:null,anthropicRefs:null,ambiguousDuplicateField:true};const ambiguousSystemBlock={kind:"source",messageNodeId:null,messageSpanStart:null,messageSpanEnd:null,sourceNodeId:8250,sourceSpanStart:700,sourceSpanEnd:760,fieldNodeId:8260,fieldSpanStart:650,fieldSpanEnd:800,category:"unknown",role:"system",roleSourceNodeId:null,roleSourceSpanStart:null,roleSourceSpanEnd:null,openaiRefs:null,anthropicRefs:null,ambiguousDuplicateField:true};const normalSiblingBlock={...block("source","unknown","assistant"),ambiguousDuplicateField:false};
const ambiguousMessagePage={blocks:[ambiguousMessageBlock,ambiguousChildBlock,ambiguousSystemBlock,normalSiblingBlock],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:8100,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:8200,candidateSpanStart:100,candidateSpanEnd:900,ambiguousDuplicateField:false}};const ambiguousMessageCalls=[];const ambiguousMessageInvoke=async(command,args)=>{ambiguousMessageCalls.push({command,args});if(command==="get_children")return {nodes:[ambiguousMessageCandidate],hasMore:false,nextCursor:null};if(command==="get_conversation_candidate")return {nodeId:8200,spanStart:100,spanEnd:900,messageCount:4,kind:"none",scopeRootId:8100,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:31,ambiguousDuplicateField:true};if(command==="get_conversation_blocks")return ambiguousMessagePage;if(command==="get_node_summary")return node(args.nodeId,"object","complete source",0,1000,0);throw new Error("unexpected ambiguous message command "+command);};
const ambiguousMessageHost=document.createElement("div");document.body.append(ambiguousMessageHost);const ambiguousMessageView=new ConversationView({panel:ambiguousMessageHost,invoke:ambiguousMessageInvoke,onError:(error)=>{throw error;},onRaw:(target)=>raw.push(target),onTree:(target)=>tree.push(target),onContent:(target)=>content.push(target)});ambiguousMessageView.setContext({mode:"document",sessionRevision:31,sourceSize:1000,scopeRoot:ambiguousMessageRoot,scopeLabel:"ambiguous messages"});await settle();await settle();const ambiguousBlocks=ambiguousMessageHost.querySelectorAll(".conversation-block");check(ambiguousBlocks.length===4&&ambiguousMessageHost.querySelectorAll(".conversation-block-ambiguity").length===3,"ambiguous message/child/system blocks or normal sibling were not preserved");check((ambiguousBlocks[0].querySelector(".conversation-block-summary")?.textContent?.includes("complete source")||ambiguousBlocks[0].querySelector(".conversation-block-summary")?.textContent?.includes("完整源对象"))===true&&ambiguousBlocks[0].textContent?.includes("unknown")===true&&ambiguousBlocks[0].querySelector('[data-conversation-action="raw"]')!==null&&ambiguousBlocks[0].querySelector("[data-conversation-ref-key]")===null,"whole-message ambiguity did not fall back to its complete source without specialized refs");check(ambiguousBlocks[1].textContent?.includes("assistant")===true&&ambiguousBlocks[1].querySelector('[data-conversation-action="role"]')!==null&&ambiguousBlocks[1].querySelector('[data-conversation-action="raw"]')!==null&&ambiguousBlocks[1].querySelector("[data-conversation-ref-key]")===null,"ambiguous child lost its normal parent role/source or exposed specialized refs");check(ambiguousBlocks[2].textContent?.includes("system")===true&&ambiguousBlocks[2].querySelector('[data-conversation-action="raw"]')!==null&&ambiguousBlocks[2].querySelector('[data-conversation-action="tree"]')!==null,"ambiguous system child did not retain its complete source entry");ambiguousBlocks[1].querySelector('[data-conversation-action="raw"]')?.click();check(raw.at(-1)?.ref.nodeId===8220&&raw.at(-1)?.ref.spanStart===200,"ambiguous child Raw action did not preserve its complete child source");ambiguousBlocks[1].querySelector('[data-conversation-action="role"]')?.click();check(raw.at(-1)?.ref.nodeId===8240,"ambiguous child role source did not remain available");check(!ambiguousBlocks[3].classList.contains("conversation-block-ambiguous")&&ambiguousBlocks[3].textContent?.includes("unknown")===true,"normal unknown sibling was incorrectly marked ambiguous");ambiguousMessageHost.remove();

const anthropicWrapperHost=document.createElement("div");document.body.append(anthropicWrapperHost);const anthropicWrapperInvoke=async(command,args)=>{if(command==="get_children")return {nodes:[ambiguousCandidate],hasMore:false,nextCursor:null};if(command==="get_conversation_candidate")return {...ambiguousCandidateDto,scopeRootId:8000,kind:"anthropic",sessionRevision:30,ambiguousDuplicateField:false};if(command==="get_conversation_blocks")return ambiguousWrapperPage;if(command==="get_node_summary")return node(args.nodeId,"object","ambiguous system wrapper",0,1000,0);throw new Error("unexpected anthropic wrapper command "+command);};const anthropicWrapperView=new ConversationView({panel:anthropicWrapperHost,invoke:anthropicWrapperInvoke,onError:(error)=>{throw error;},onRaw:(target)=>raw.push(target),onTree:(target)=>tree.push(target),onContent:(target)=>content.push(target)});anthropicWrapperView.setContext({mode:"document",sessionRevision:30,sourceSize:1000,scopeRoot:ambiguousRoot,scopeLabel:"ambiguous system"});await settle();await settle();check(anthropicWrapperHost.querySelectorAll(".conversation-block").length===1&&anthropicWrapperHost.querySelectorAll(".conversation-block-ambiguity").length===1&&anthropicWrapperHost.querySelector('[data-conversation-action="wrapper-raw"]')!==null,"Anthropic-style wrapper ambiguity was rejected instead of using its complete scope source");anthropicWrapperHost.remove();

const malformedCandidateHost=document.createElement("div");document.body.append(malformedCandidateHost);const malformedCandidateInvoke=async(command,args)=>{if(command==="get_children")return {nodes:[ambiguousCandidate],hasMore:false,nextCursor:null};if(command==="get_conversation_candidate")return {...ambiguousCandidateDto,ambiguousDuplicateField:"yes"};throw new Error("unexpected malformed candidate command "+command);};const malformedCandidateView=new ConversationView({panel:malformedCandidateHost,invoke:malformedCandidateInvoke,onError:(error)=>{throw error;},onRaw:()=>{},onTree:()=>{},onContent:()=>{}});malformedCandidateView.setContext({mode:"document",sessionRevision:30,sourceSize:1000,scopeRoot:ambiguousRoot,scopeLabel:"malformed flag"});await settle();await settle();check(malformedCandidateHost.querySelectorAll(".conversation-block").length===0&&malformedCandidateHost.textContent.includes("invalid"),"malformed candidate ambiguousDuplicateField escaped candidate validation");malformedCandidateHost.remove();

const malformedBlockHost=document.createElement("div");document.body.append(malformedBlockHost);const malformedBlockInvoke=async(command,args)=>{if(command==="get_children")return {nodes:[ambiguousMessageCandidate],hasMore:false,nextCursor:null};if(command==="get_conversation_candidate")return {nodeId:8200,spanStart:100,spanEnd:900,messageCount:1,kind:"generic",scopeRootId:8100,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:31,ambiguousDuplicateField:false};if(command==="get_conversation_blocks")return {...ambiguousMessagePage,blocks:[{...ambiguousChildBlock,ambiguousDuplicateField:"yes"}]};throw new Error("unexpected malformed block command "+command);};const malformedBlockView=new ConversationView({panel:malformedBlockHost,invoke:malformedBlockInvoke,onError:(error)=>{throw error;},onRaw:()=>{},onTree:()=>{},onContent:()=>{}});malformedBlockView.setContext({mode:"document",sessionRevision:31,sourceSize:1000,scopeRoot:ambiguousMessageRoot,scopeLabel:"malformed block flag"});await settle();await settle();check(malformedBlockHost.querySelectorAll(".conversation-block").length===0&&malformedBlockHost.textContent.includes("invalid"),"malformed block ambiguousDuplicateField escaped page validation");malformedBlockHost.remove();

const malformedBoundaryHost=document.createElement("div");document.body.append(malformedBoundaryHost);const malformedBoundaryInvoke=async(command,args)=>{if(command==="get_children")return {nodes:[ambiguousMessageCandidate],hasMore:false,nextCursor:null};if(command==="get_conversation_candidate")return {nodeId:8200,spanStart:100,spanEnd:900,messageCount:1,kind:"generic",scopeRootId:8100,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:31,ambiguousDuplicateField:false};if(command==="get_conversation_blocks")return {...ambiguousMessagePage,blocks:[{...ambiguousChildBlock,sourceSpanEnd:1200}]};throw new Error("unexpected malformed source-boundary command "+command);};const malformedBoundaryView=new ConversationView({panel:malformedBoundaryHost,invoke:malformedBoundaryInvoke,onError:(error)=>{throw error;},onRaw:()=>{},onTree:()=>{},onContent:()=>{}});malformedBoundaryView.setContext({mode:"document",sessionRevision:31,sourceSize:1000,scopeRoot:ambiguousMessageRoot,scopeLabel:"malformed source boundary"});await settle();await settle();check(malformedBoundaryHost.querySelectorAll(".conversation-block").length===0&&malformedBoundaryHost.textContent.includes("invalid"),"ambiguous source reference escaped scope-boundary validation");malformedBoundaryHost.remove();
...
const inlineBlock=(id,category="content",role="assistant",kind="source",style="generic",refs={})=>({
  ...
  anthropicRefs:style==="anthropic"?{block:inlineRef(id),text:refs.text??null,thinking:refs.thinking??null,data:refs.data??null,id:null,name:null,input:null,toolUseId:null,content:null}:null,ambiguousDuplicateField:false
});
...
const inlineInvoke=async(command,args)=>{
  ...
  if(command==="get_conversation_candidate")return {nodeId:800,spanStart:0,spanEnd:500000,messageCount:31,kind:"generic",scopeRootId:800,scopeRootSpanStart:0,scopeRootSpanEnd:500000,sessionRevision:18,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks"){
    const blocks=args.style==="openai"?openaiInlineBlocks:args.style==="anthropic"?anthropicInlineBlocks:genericInlineBlocks;
    return {blocks,hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:800,scopeRootSpanStart:0,scopeRootSpanEnd:500000,candidateNodeId:800,candidateSpanStart:0,candidateSpanEnd:500000,ambiguousDuplicateField:false}};
  }
...
const headInvoke=async(command,args)=>{
  ...
  if(command==="get_conversation_candidate")return {nodeId:950,spanStart:0,spanEnd:500000,messageCount:20,kind:"generic",scopeRootId:950,scopeRootSpanStart:0,scopeRootSpanEnd:500000,sessionRevision:19,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return {blocks:headBlocks,hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:950,scopeRootSpanStart:0,scopeRootSpanEnd:500000,candidateNodeId:950,candidateSpanStart:0,candidateSpanEnd:500000,ambiguousDuplicateField:false}};
...
const deferredInvoke=async(command,args)=>{
  if(command==="get_conversation_candidate")return {nodeId:970,spanStart:0,spanEnd:500000,messageCount:1,kind:"generic",scopeRootId:970,scopeRootSpanStart:0,scopeRootSpanEnd:500000,sessionRevision:20,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return {blocks:[deferredBlock],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:970,scopeRootSpanStart:0,scopeRootSpanEnd:500000,candidateNodeId:970,candidateSpanStart:0,candidateSpanEnd:500000,ambiguousDuplicateField:false}};
  if(command==="get_node_summary")return node(args.nodeId,"string","content",args.nodeId*10,args.nodeId*10+5,0);
...
const toolInvoke=async(command,args)=>{
  ...
  if(command==="get_conversation_candidate")return {nodeId:1200,spanStart:0,spanEnd:500000,messageCount:8,kind:"generic",scopeRootId:1200,scopeRootSpanStart:0,scopeRootSpanEnd:500000,sessionRevision:21,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return {blocks:toolBlocks,hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:1200,scopeRootSpanStart:0,scopeRootSpanEnd:500000,candidateNodeId:1200,candidateSpanStart:0,candidateSpanEnd:500000,ambiguousDuplicateField:false}};
...
const largeInvoke=async(command,args)=>{
  if(command==="get_conversation_candidate")return {nodeId:101,spanStart:0,spanEnd:100000,messageCount:10000,kind:"generic",scopeRootId:101,scopeRootSpanStart:0,scopeRootSpanEnd:100000,sessionRevision:8,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return {blocks:largeBlocks,hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:101,scopeRootSpanStart:0,scopeRootSpanEnd:100000,candidateNodeId:101,candidateSpanStart:0,candidateSpanEnd:100000,ambiguousDuplicateField:false}};
...
const scanInvoke=async(command,args)=>{
  ...
  if(command==="get_conversation_candidate"){if(args.candidateNodeId===301)throw {code:"invalid_request",message:"literal key is not a candidate"};if(args.candidateNodeId===302)return {nodeId:302,spanStart:110,spanEnd:800,messageCount:2,kind:"generic",scopeRootId:300,scopeRootSpanStart:0,scopeRootSpanEnd:100000,sessionRevision:9,ambiguousDuplicateField:false};return {nodeId:310,spanStart:90000,spanEnd:99999,messageCount:2,kind:"generic",scopeRootId:300,scopeRootSpanStart:0,scopeRootSpanEnd:100000,sessionRevision:9,ambiguousDuplicateField:false};}
  if(command==="get_conversation_blocks"){const candidate=args.candidateNodeId===302?earlyCandidate:lateCandidate;return {blocks:[],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:300,scopeRootSpanStart:0,scopeRootSpanEnd:100000,candidateNodeId:candidate.id,candidateSpanStart:candidate.spanStart,candidateSpanEnd:candidate.spanEnd,ambiguousDuplicateField:false}};}
...
const invalidHost=document.createElement("div");document.body.append(invalidHost);const invalidRoot=node(401,"array","messages",0,1000,2);const invalidInvoke=async(command,args)=>{
  if(command==="get_conversation_candidate")return {nodeId:401,spanStart:0,spanEnd:1000,messageCount:2,kind:"generic",scopeRootId:401,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:10,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return {blocks:[{...block("message"),messageNodeId:null,messageSpanStart:null,messageSpanEnd:null}],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:401,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:401,candidateSpanStart:0,candidateSpanEnd:1000,ambiguousDuplicateField:false}};
...
const lateInvoke=async(command,args)=>{
  if(command==="get_conversation_candidate")return {nodeId:args.scopeRootId,spanStart:0,spanEnd:1000,messageCount:2,kind:"generic",scopeRootId:args.scopeRootId,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:args.sessionRevision,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return {blocks:[lateBlock(args.scopeRootId===501?601:602)],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:args.scopeRootId,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:args.candidateNodeId,candidateSpanStart:0,candidateSpanEnd:1000,ambiguousDuplicateField:false}};
...
const focusInvoke=async(command,args)=>{
  if(command==="get_conversation_candidate")return {nodeId:701,spanStart:0,spanEnd:1000,messageCount:2,kind:"generic",scopeRootId:701,scopeRootSpanStart:0,scopeRootSpanEnd:1000,sessionRevision:13,ambiguousDuplicateField:false};
  if(command==="get_conversation_blocks")return new Promise((resolve)=>{releaseFocusPage=()=>resolve({blocks:[],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:701,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:701,candidateSpanStart:0,candidateSpanEnd:1000,ambiguousDuplicateField:false}});});
...
window.__TAURI_INTERNALS__={invoke:async(command,args)=>{
  ...
  if(command==="get_conversation_blocks")return {blocks:[block("source","content","user")],hasMore:false,nextCursor:null,wrapperRef:{scopeRootId:1,scopeRootSpanStart:0,scopeRootSpanEnd:1000,candidateNodeId:10,candidateSpanStart:100,candidateSpanEnd:900,ambiguousDuplicateField:false}};
